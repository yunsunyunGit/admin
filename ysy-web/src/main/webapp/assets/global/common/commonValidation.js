...
function commNulMaxLenCheck1(ctrlId,len,errMsg) {
    return doCombiValidation(ctrlId,
            {
                "vsymbol" : {message : errMsg	},
                "maxlen": {lenmax : len,message : errMsg}
...
/**
 * 常用非空验证组合, 包括验证长度上限(没有下限)
 */
function commLengthCheck(ctrlId,len,errMsg) {
    return doCombiValidation(ctrlId,
            {
                "maxlen" : {lenmax : len,message : errMsg	}
            });
...
function idCardCheck(ctrlId, errMsg) {
    ...
    if(cardId != "" && cardId != null && typeof cardId != "undefined"){
        if(cardId.length!=15 && cardId.length!=18){
            return processFailure("idnum",errMsg,14);
        }
        if(isNaN(cardId)){
            ...
            if((isNaN(firstCard)) || endCard!="X"){
                return processFailure("idnum",errMsg,13);
            }
        }
    }
...
function doSymbolValidNotUnderAndNumber(id, message){
    ...
    if(!result.test(v)){
        return processFailure(id, message, 26);
    }
    //return processSuccess(id);
    return successMsg(id);
}
...
function doNumberValidationIsNum(id,message){
    var v = jQuery('#' + id).val();
    if(v == null|| v == ''){
        return successMsg(id);
    }
    var teat =/^([\d]*[.]{0,1}[\d]*)$/;  ///^[1-9]+[0-9]$/;
    //var result=v.match(g);
    if(!teat.test(v)){
...
function doCombiValidation(id, obj) {
    ...
    // 数字
    if(obj.numberIsNotNull != null){
        if(!doNumberValidationIsNum(id,obj.numberIsNotNull.message)){
            return false;
        }
    }
...
function errorMsg(id, message, fn) {
    ...
    case 18 :
        ...
        break;
    case 19 :
        msg = message + "手机只能以13,15,18开头且长度为11位数字";
        break;
    case 20 :
